...
export function AllowedSignupDomainsTable({
  domains,
}: {
  domains: DomainRow[];
}) {
  ...
  async function handleDelete(id: number) {
    if (!confirm("Delete this allowed signup domain? This cannot be undone."))
      return;
    const err = await deleteAllowedSignupDomainAction(id);
    if (err) {
      alert(err);
      return;
    }
  ...
  if (domains.length === 0) {
    return (
      <div className="rounded-xl border border-zinc-200 bg-white py-16 text-center shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <p className="text-zinc-500">No allowed signup domains found.</p>
      </div>
    );
  }
