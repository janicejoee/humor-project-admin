...
import { WhitelistEmailsTable } from "./whitelist-emails-table";

const PAGE_SIZE = 25;

export default async function AdminWhitelistEmailsPage({
  searchParams,
}: {
  ...
  if (error) {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-800 dark:border-red-800 dark:bg-red-950 dark:text-red-200">
        Failed to load whitelisted emails: {error.message}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link
          href="/admin/whitelist-emails/new"
      ...
      <WhitelistEmailsTable emails={emails ?? []} />

      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          {page > 1 && (
            <Link
              href={`/admin/whitelist-emails?page=${page - 1}`}
