...
export default async function AdminLlmModelsPage({
  ...
  const modelsWithProvider = (models ?? []).map((m) => ({
    ...m,
    llm_providers: Array.isArray(m.llm_providers) ? m.llm_providers[0] : m.llm_providers,
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      <LlmModelsTable models={modelsWithProvider} />

      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          {page > 1 && (
            <Link
              href={`/admin/llm-models?page=${page - 1}`}
